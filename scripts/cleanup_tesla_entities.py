#!/usr/bin/env python3
"""
Delete orphaned tesla_* sensor/binary_sensor entities from Home Assistant
after the rename to tesla_* alias layer.

Keeps all input_* helpers, scripts, and the weekly_distance_2 sensor
(it's still referenced as a fallback until HA re-registers the new weekly one).

Usage:
  python3 scripts/cleanup_tesla_entities.py --url https://your-homeassistant-url --token YOUR_TOKEN
...
# Domains to remove (these are now renamed to sensor.tesla_* / binary_sensor.tesla_*)
DELETE_DOMAINS = {"sensor", "binary_sensor"}

# Specific entity IDs to KEEP even if they match tesla_ in the above domains
...
def main():
    parser = argparse.ArgumentParser(description="Clean up orphaned tesla entities")
    ...
    candidates = [
        e for e in entities
        if "tesla" in e["entity_id"]
        and e["entity_id"].split(".")[0] in DELETE_DOMAINS
        and e["entity_id"] not in KEEP_ENTITIES
    ]

    if not candidates:
        print("✅ No orphaned tesla sensor/binary_sensor entities found.")
        ws.close()
        return
